<x-layout>
    ...
    <section class="w-4/5 mx-auto mt-3 bg-gray-600 text-white p-3">
        ...
        @if ($reqs->count())
            ...
            <section class="my-3 px-5 grid grid-cols-2" x-data="{ open: false }">
                <section class="text-2xl col-span-2 mb-12 flex items-center justify-around" x-data="{ text: open ? 'hide' : 'show' }">
                    <h2>
                        Filters
                    </h2>
                    <button class="py-2 px-4 bg-blue-500 rounded-xl text-base" x-on:click="open = ! open"
                        @click="$event.target.innerHTML = open ? 'hide' : 'show' ">show</button>
                </section>
                ...
                <section class="col-span-2 w-4/5 grid grid-cols-2 mx-auto" x-show="open" x-transition>
                    ...
                    <select name="term" id="term" class="text-black my-3 p-1 term-select">

                        <option value="" selected>All</option>

                        @foreach ($allReqs->unique('term_id') as $item)
                            @if (request('term') && $item->term_id == request('term'))
                                <option value="{{ $item->term_id }}" selected>
                                    {{ $item->term->name . ' : ' . $item->term->created_at->format('Y-m') }}</option>
                            @else
                ...
                <tbody>
                    ...
                    @foreach ($reqs as $item)
                        ...
                        <tr class="text-center">
                            ...
                            <td class="p-3">{{ $item->doctorOfSubject->subject->name }}</td>
    ...
    <a href="/student/{{ $student->id }}/request/create"
